var backgrounds = ['img/site/loader/bg01.jpg', 'img/site/loader/bg02.jpg', 'img/site/loader/bg03.jpg', 'img/site/loader/bg04.jpg', 'img/site/loader/bg05.jpg', 'img/site/loader/bg06.jpg', 'img/site/loader/bg07.jpg', 'img/site/loader/bg08.jpg'];
var phrases = ['img/site/loader/tip01.png', 'img/site/loader/tip02.png', 'img/site/loader/tip03.png', 'img/site/loader/tip04.png', 'img/site/loader/tip05.png', 'img/site/loader/tip06.png', 'img/site/loader/tip07.png', 'img/site/loader/tip08.png'];
var change = {
    src = "/index.html",
    time: 10000
};
var date = new DateTime();
date.CheckCookies(document.cookie.split(';'));

function DateTime() {
    var date;
    this.CheckCookies = function(cookies) {
        for (var i = 0; i < cookies.length; i++) {
            var key = cookies[i].split('=');
            if (key[0].replace(' ', '') == 'CGBackground' || key[0].replace(' ', '') == 'CGPhrase') {
                this.ChangePage();
                return true;
            } else if (i + 1 == cookies.length)
                this.SetCookie(this.GetTime(), this.SortCookies());
        }
    }
    this.ChangePage = function() {
        window.location = window.location.href + change.page;
    }
    this.GetTime = function() {
        this.date = new Date();
        return this.date.toLocaleTimeString();
    };
    this.SortCookies = function() {
        var types = {
            bg: Math.round(Math.random() * (backgrounds.length - 1)),
            phrase: Math.round(Math.random() * (phrases.length - 1))
        };
        return types;
    };
    this.SetCookie = function(time, types) {
        var background = "CGBackground=" + backgrounds[types.bg] + "; expires=" + this.date.toString();
        var phrase = "CGPhrase=" + phrases[types.phrase] + "; expires=" + this.date.toString();
        document.cookie = background.replace(time, '23:59:59');
        document.cookie = phrase.replace(time, '23:59:59');
        this.SetFiles(backgrounds[types.bg], phrases[types.phrase]);
    };
    this.SetFiles = function(bg, phrase) {
        var loader = document.getElementsByClassName('loader')[0];
        var tip = document.getElementsByClassName('tip')[0];
        loader.style.backgroundImage = 'url(' + bg + ')';
        tip.style.backgroundImage = 'url(' + phrase + ')';
        setTimeout(this.ChangePage, change.time);
    };
};;
